...
void RunBruteForceSearch(FILE *fp, raw_data_t *rp, index_t *ip, query_t *qp, int tau)
{  
  int l;
  char linebuf[MAX_DOC_LEN + 128];
  memset(linebuf, PREFIX_PATCH_CHAR, rp->raw_q);
  ...
  while (fgets(line, MAX_DOC_LEN, fp)){    
    ...
    for (int i = 0; i < rp->raw_doc_num; i ++){
      candNum ++;
      int k = edit_distance(llen, line, rp->raw_doc_len[i], rp->raw_doc_str[i], tau);
      if (k <= tau) {
        resultNum ++;
      }
    }       
...
int main(int argc, char* argv[])
{
  ...
  RunBruteForceSearch(stdin, rp, ip, qp, tau);
